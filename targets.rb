...
class Build    # main class and namespace
  ...
  class Linkable < CompileOrLinkTarget   # common base class for libs and executables
    ...
    #
    # list -- list of paths to objects to link
    # libs -- list of library objects whose paths we want to add
    #
    def self.add_libs list, libs
  ...
  class LibFileTarget < Linkable    # library (static or dynamic)
    ...
    def get_cmd
      ...
      # for a library, we only need the subsidiary libraries in the case of dynamic
      # linking since the static library is built using "ar" and only needs the local
      # objects
      #
      objlist = @deps.map( &:path )
      if :dynamic == @link_type && defined? @libs    # add lib files if any
      ...
      # error if -shared is not present for a dynamic library
      i = opt.get_opt '-shared'
      j = opt.get_post '-shared'
      raise "Library target %s missing -shared linker option" % @path if ( !i && !j )
      raise "Library target %s missing soname" % @path if !defined? @soname
  ...
  class ExecFileTarget < Linkable    # executable
    ...
    # args is a hash with these keys:
    #   options -- array of options to add [required]
    #   type    -- type of options (:exe) [optional];
    #   replace -- if false, an exception is raised if an existing option conflicts with
    #              new options; if true, such conflicting options are removed; default is
    #              false [optional]
    #

...
%w{ build options targets snappy_config features }.each{ |f| require f }
...
class Build
  ...
  class Snappy < Bundle
    ...
    def initialize build    # see notes at Bundle.initialize
      ...
      @exclude  = %w{ .deps .libs m4 testdata}
      ...
      #
      @libraries = [{ :name   => 'libsnappy',
                      :files  => %w{ snappy snappy-c snappy-sinksource
                                     snappy-stubs-internal },
                      :linker => :ld_cxx }]

      # an executable is defined by a name, list of objects and libraries and type of link
      @executables = [{ :name   => 'snappy_unittest',
                        :files  => ['snappy_unittest', 'snappy-test'],
                        :libs   => ['libsnappy'],
                        :linker => :ld_cxx }]

      # default targets to build
      @targets = ['libsnappy', :lib, 'snappy_unittest', :exe]
    end  # initialize
